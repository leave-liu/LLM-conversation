...
export const cn_proxy_url = "http://172.28.210.75:5173/cnproxy";
// Overseas Coze agent address
export const ncn_proxy_url = "http://172.28.210.75:5173/ncnproxy";

const redirect_uri = "http://172.28.210.75:3000/";
export const asyncChat = async (
  messages: MessageApiInter[],
  abort: AbortController
) => {
  return await fetch('https://api.coze.cn/v3/chat', {
    method: "POST",
    headers: {
...
export const asyncFileUpload = async (file: File) => {
  try {
    ...
    if (jsonData && jsonData.code == 700012006 && getStorageSetting()?.auth_type === "two") {
